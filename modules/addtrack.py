def add_tracks(self, arg):
    ...
    if len(matches) > 1:
        import random
        self.conman.gen_send("%s results for search \"%s\"; selecting one at random" % (len(matches), arg))
        rand = random.randint(0, len(matches)-1)
        self.conman.gen_send("Adding %s" % matches[rand]["file"])
        self.conman.mpc.add(matches[rand]["file"])
    elif len(matches) == 0:
        self.conman.gen_send("No songs matching \"%s\"" % arg)
    else:

def youtube(self, string):
    ...
    if len(stuff) > 0:
        ...
        for match in stuff:
            ...
            try:
                jdata = jdata["data"]
                self.conman.gen_send("Youtube: %s - Uploaded by %s - Uploaded %s - %s views" % (jdata["title"].encode('utf-8'), jdata["uploader"].encode('utf-8'), jdata["uploaded"].encode('utf-8'), jdata["viewCount"]))
            except KeyError:
